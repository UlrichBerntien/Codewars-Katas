...
encode:
    ...
    push   table_encode     ; address of the translation table
    jmp    translate        ; do translation and stack rewind
; ---> endof encode <---
...
decode:
    push   rbp              
    push   rbx              ; save registers
    push   table_decode     ; address of the translation table
    jmp    translate        ; do translation and stack rewind
; ---> endof decode <---

; <-- translate -->
; input:
;    rax - pointer to string to translate
;    stack: address translation table, old rbx, old rbp
; output:
;    rax - pointer to new allocated buffer.
;          contains the translated string.  
translate:
    mov    rbp, rdi         ; save input string address
    call   strlen
    inc    rax              ; add 1 byte to store the \0
    mov    rdi, rax
    call   malloc           ; allocate buffer for translated string
    mov    rdi, rax         ; rdi = base address output string
    mov    rsi, rbp         ; rsi = base address input string
    xor    rcx, rcx         ; index in both strings
    pop    rbx              ; base address of the char translation table
.loop:    
    movzx  rax,byte [rsi+rcx] ; load char from input string
    mov    al,[rbx+rax]     ; translate the char
    mov    [rdi+rcx],al     ; store in destination string
    inc    rcx              ; next char
.start:    
    test   al,al            ; 0 terminates the string
    jnz    .loop
    mov    rax, rdi         ; return address of new string
    pop    rbx              ; rewind stack, restore registers
    pop    rbp
    ret
; ---> endof translate <---
...
; Table to encode vowels to numbers
table_encode:
    ...
    db 0x60, 0x31, 0x62, 0x63, 0x64, 0x32, 0x66, 0x67
    db 0x68, 0x33, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x34
    db 0x70, 0x71, 0x72, 0x73, 0x74, 0x35, 0x76, 0x77
...
; table to decode numbers to vowels
table_decode:
    db 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07
    db 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
    db 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17
    ...
    db 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27
    db 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f
    db 0x30, 0x61, 0x65, 0x69, 0x6f, 0x75, 0x36, 0x37
    db 0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f
    db 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47
    db 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f
    db 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57
